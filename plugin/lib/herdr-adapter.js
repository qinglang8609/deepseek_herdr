...
export class HerdrAdapter {
	 ...
	 * @param {string[]} [args] - extra AGENT_ARGS forwarded after `--`
	 *   (e.g. `["--resume", "<sessionId>"]` for claude, `["-s", "<id>"]` for
	 *   opencode, `["resume", "<id>"]` for codex).
	 */
	async agentStart(name, kind, paneId, { timeoutMs = 60000, args = null } = {}) {
		const argv = ["agent", "start", name, "--kind", kind, "--pane", paneId, "--timeout", String(timeoutMs)];
		if (Array.isArray(args) && args.length > 0) argv.push("--", ...args.map(String));
		return this.call(argv, { timeoutMs: timeoutMs + 5000 });
	}
	/**
